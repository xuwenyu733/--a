function buildHeaderMetaFromForm(form) {
  ...
  const gradTime =
    latestEdu.end?.trim() || [latestEdu.start, latestEdu.end].filter(Boolean).join(' – ') || '';
  ...
  return {
    ...
    major: latestEdu.major?.trim() || '',
    degree: latestEdu.degree?.trim() || '',
    school: latestEdu.school?.trim() || '',
    gradTime,
    advantage,
...
function headerMetaToGridMap(meta) {
  if (!meta) return {};
  return {
    年龄: meta.age,
    电话: meta.phone,
...
function parseHeaderMetaFromMarkdown(markdown) {
  const md = markdown || '';
...
function normalizeResumeHeader(markdown, meta) {
...
function resolveHeaderMeta(markdown, builderData, photoUrl) {
